    ...
    <nav className="site-nav sticky top-0 z-50">
      <div className="mx-auto flex max-w-7xl items-center gap-0.5 px-2 sm:gap-2 sm:px-6">
        <Link href={`/${locale}`} className="nav-brand flex min-h-[64px] items-center">
          <BrandMark width={126} priority className="w-[58px] min-[360px]:w-[72px] min-[420px]:w-[94px] sm:w-[126px]" />
        </Link>
        ...
        <div className="ms-auto flex items-center gap-0.5 sm:gap-1">
          {links.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`nav-link min-h-[44px] items-center rounded-full px-1 text-[10px] font-semibold text-[var(--ink-muted)] min-[420px]:px-1.5 min-[420px]:text-[11px] sm:px-4 sm:text-sm ${link.mobileHidden ? 'hidden sm:flex' : 'flex'}`}
            >
              {link.label}
            </Link>
          ))}
