    ...
    <nav className="site-nav sticky top-0 z-50">
      <div className="mx-auto flex max-w-7xl items-center gap-1 px-3 sm:gap-2 sm:px-6">
        <Link href={`/${locale}`} className="nav-brand flex min-h-[64px] items-center">
          <BrandMark width={126} priority className="w-[105px] sm:w-[126px]" />
        </Link>
        ...
        <div className="ms-auto flex items-center gap-0.5 sm:gap-1">
          {links.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="nav-link flex min-h-[44px] items-center rounded-full px-2.5 text-xs font-semibold text-[var(--ink-muted)] sm:px-4 sm:text-sm"
            >
              {link.label}
            </Link>
          ))}
